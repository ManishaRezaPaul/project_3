...
function init() {
    ...
    d3.json(url).then((matchData) => {
        ...
        function updateSunburst() {
            ...
            let data = [{
                type: 'sunburst',
                labels: yearData.map(item => [item.Home_Team, item.Away_Team]),
                domain: {column: 0},
                parents: yearData.map(item => selectedYear),
                values: yearData.map(item => parseFloat([item.Home_Goals])),
                insidetextorientation: 'radial'

            },{
                type:'sunburst',
                labels: yearData.map(item => [item.Away_Team, item.Home_Team]),
                domain: {column: 1},
                parents: yearData.map(item => selectedYear),
                values: yearData.map(item => parseFloat([item.Away_Goals])),
                insidetextorientation: 'radial'
            }];
        ...
        am5core.ready(function() {
            let chart = am5core.create("chartContainer", am5charts.PieChart);
            let selectedYear = +dropdownmenu.property("value"); // Convert to number
    ...
    //     var chart = am5core.create("chartContainer", am5charts.PieChart);
    ...
    //         // Create series
    //         var series = chart.series.push(new am5charts.PieSeries());
    //         series.dataFields.value = "Home_Goals";
    //         series.dataFields.category = "Home_Team";
